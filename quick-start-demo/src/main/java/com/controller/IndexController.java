...
public class IndexController {
	...
	public Map<String,Object> login(@RequestParam(value = "corpId") String corpId,
		...
		// 用户detail
		ServiceResult<UserDetailDto> user = httpSDK.getLoginUser(userId,corpToken);
		System.out.println(user.getResult().toString());
		// 通讯录权限
		ServiceResult<JSONObject> permissions = httpSDK.checkBookPermissions(corpToken);
		System.out.println(permissions.getResult().toJSONString());
		// 部门列表
		ServiceResult<JSONObject> departments = httpSDK.getAllDepartment(corpToken, String.valueOf(1));
		System.out.println(departments.getResult().toJSONString());
		//返回结果
